...
//        if ($selected_category_ids) {
        ...
        foreach ($children as $category_id) {
            ...
            if ($subcat && !is_wp_error($subcat)) {
                ...
                ?>
                <a class="swiper-slide rounded-3 shadow-sm row align-items-end p-4"
                   style="height:350px; background:url('<?= $thumbnail_id['url'] ?>')"
                   href="<?= esc_url(get_term_link($subcat, $subcat->taxonomy)); ?>/#6cols_product">
                    <h4 class="text-white bg-dark bg-opacity-75 p-3 rounded-2 fs-4 fw-bolder mb-0 d-flex justify-content-between"><?= $subcat->name; ?>
