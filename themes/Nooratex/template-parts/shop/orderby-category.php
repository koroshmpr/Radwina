<form class="w-100 woocommerce-ordering border-bottom border-info border-opacity-50 d-flex gap-2 align-content-center" method="get">
    <h5 class="border-bottom border-2 border-primary w-auto py-1 mb-0 fw-bold fs-6 col-3 col-lg-auto">مرتب سازی بر اساس :</h5>
    <div class="d-flex gap-3 align-items-center text-dark overflow-x-scroll orderby" aria-label="<?php esc_attr_e('Shop order', 'woocommerce'); ?>">
        ...
        <?php foreach ($orderby_options as $orderby_value => $orderby_label) :
                ...
                $class = ($current_orderby === $orderby_value) ? ' border-bottom ps-2 border-2 border-warning' : '';
                ?>
